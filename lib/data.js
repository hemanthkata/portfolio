...
export const mlProjects = [
  {
    slug: "docurag-pdf-qa",
    name: "DocuRAG — PDF Document Q&A Pipeline",
    tag: "NLP / LLM · RAG",
    year: "2025",
    role: "ML Engineer",
    desc: "End-to-end RAG system for querying PDF documents — text extraction, chunking, ChromaDB vector storage, semantic search, and LLM-grounded answers.",
    overview:
      "A production Retrieval-Augmented Generation pipeline that ingests PDF documents, extracts and chunks text, stores embeddings in ChromaDB for semantic search, and injects retrieved context into an LLM for grounded, document-specific answers — preventing hallucination on private knowledge bases.",
    features: [
      "PDF text extraction and intelligent chunking pipeline",
      "Embedding generation and semantic similarity search",
      "ChromaDB vector database for persistent embedding storage",
      "Retrieved-context injection into LLM for grounded, document-specific answers",
      "Local inference via Ollama (Llama-family models) and OpenAI API support",
      "Minimal hallucination on private knowledge bases",
    ],
    metrics: [
      { value: "RAG", label: "Grounded answers" },
      { value: "ChromaDB", label: "Vector store" },
      { value: "Ollama", label: "Local LLM" },
    ],
    stack: ["Python", "ChromaDB", "Ollama", "LLaMA", "OpenAI API", "Sentence-Transformers"],
  },
  {
    slug: "ai-career-coach-api",
    name: "AI Career Coach API",
    tag: "LLM · FastAPI",
    year: "2025",
    role: "ML Engineer",
    desc: "FastAPI REST API that generates structured career-transition guidance — strengths, skill gaps, a 3-month roadmap, and common pitfalls — by prompt-engineering a locally hosted Qwen3-8B LLM to return strict JSON.",
    overview:
      "A REST API that leverages prompt-engineering on a locally hosted LLM (Ollama, Qwen3-8B) to produce structured career-transition guidance. Pydantic validates all requests and enforces machine-parseable JSON responses, with a companion conversational chat endpoint for follow-up questions.",
    features: [
      "Structured career-transition output: current strengths, skill gaps, 3-month roadmap, common pitfalls",
      "Prompt-engineering to force strict machine-parseable JSON from a local LLM",
      "Zero external API cost — locally hosted inference via Ollama (Qwen3-8B)",
      "Pydantic request/response validation with FastAPI",
      "Companion conversational chat endpoint for follow-up questions",
    ],
    metrics: [
      { value: "Qwen3-8B", label: "Local LLM" },
      { value: "Structured", label: "JSON output" },
      { value: "FastAPI", label: "REST API" },
    ],
    stack: ["Python", "FastAPI", "Ollama", "Qwen3-8B", "Pydantic"],
  },
  {
...
export const projectDetails = {
  ...
  "docurag-pdf-qa": {
    challenge:
      "LLMs hallucinate and cannot answer questions about private PDF documents they were never trained on.",
    approach: [
      "Built a PDF ingestion pipeline that extracts text, splits it into overlapping chunks, and generates embeddings.",
      "Stored and indexed all embeddings in ChromaDB for fast persistent semantic search.",
      "At query time, retrieved the most relevant chunks and injected them as context into the LLM prompt.",
      "Supported both local inference (Ollama, Llama-family) and OpenAI API as interchangeable backends.",
    ],
    impact:
      "Lets users ask natural-language questions over thousands of PDF pages and receive grounded, document-specific answers with minimal hallucination.",
  },
  "ai-career-coach-api": {
    challenge:
      "Career changers need structured, actionable guidance but generic LLM responses lack the machine-parseable format required for downstream tooling.",
    approach: [
      "Designed a FastAPI REST endpoint that accepts a user's background and target role via Pydantic-validated request models.",
      "Engineered prompts that instruct the locally hosted Qwen3-8B (Ollama) to respond exclusively in strict JSON.",
      "Output schema covers: current strengths, skill gaps, a 3-month roadmap, and common pitfalls.",
      "Added a companion /chat endpoint for freeform follow-up conversation with the same local model.",
    ],
    impact:
      "Delivered structured, machine-parseable career guidance at zero API cost, running entirely on local hardware via Ollama.",
  },
